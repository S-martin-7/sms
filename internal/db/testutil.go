...
func WithTestDB(t *testing.T) *pgxpool.Pool {
	...
	_, err = pool.Exec(ctx, `TRUNCATE contact_list_members, contact_lists, contacts, events, inbound_messages, inbound_numbers, webhook_deliveries, webhook_endpoints, messages, audit_log, admin_users, api_keys, tenants RESTART IDENTITY CASCADE`)
	if err != nil {
